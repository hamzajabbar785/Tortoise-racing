...
// The result will be an array [hour, min, sec] which is the time needed in hours, minutes and seconds (round 
// down to the nearest second) or a string in some languages.
...
const race = (v1, v2, g) => {
    ...
    let differncePerHour = v2 - v1
    let hours = Math.floor(g/differncePerHour)
    g -= hours * differncePerHour
    let mints = Math.floor((g/differncePerHour) * 60)
    g -= (mints * (differncePerHour/60))
    let secs = Math.floor((g/differncePerHour) * 3600)
...
const race = (v1, v2, g) => {
    ...
    let mints = Math.floor((time * 60) % 60)
